...
function getPasswordOptions() {
  // variable to store length of password from user input.
   ...
   // show you a warning in case of you type letters instead of numbers
   if(isNaN(length) === true){
    alert('Password length must be provided as number');
    return;
   }

  // show you a warning when password is less than 8 characteres
   if(length < 8) {
    alert('Password length must be at least 8 characteres');
    return;
   }

   // show you a warning when password exceed 128 characteres
   if(length > 128) {
    alert('Password length must not exceed 128 characteres');
    return;
   }

   // variable to include special characteres
   let hasSpecialCharacters = confirm(
    'Click OK to confirm including special characters'
   )

    // variable to include numeric characteres
   let hasNumericCharacters = confirm(
    'Click OK to confirm including numeric characters'
   )

    // variable to include lowercase characteres
   let hasLowerCasedCharacters = confirm(
    'Click OK to confirm including lowercase characters'
   )

    // variable to include uppercase characteres
   let hasUpperCasedCharacters = confirm(
    'Click OK to confirm including uppercase characters'
   )
   // show you a warning when you have no choose at least one option
...
// Variable password options to create the Password with all caracteristics
 let passwordOptions = {
  length: length,
